...
export const translateSentencesBatch = async (sentences: string[], targetLang: string, apiKey: string) => {
  ...
  const response = await ai.models.generateContent({
    model: 'gemini-3-flash-preview',
    contents: `Translate the following JSON array of strings to ${targetLang}. If ${targetLang} is Hebrew, you MUST translate it to Hebrew (עברית). Return ONLY a valid JSON array of strings with the exact same number of elements, where each element corresponds to the translated string at the same index. Do not include markdown formatting like \`\`\`json.\n\n${JSON.stringify(sentences)}`,
    config: {
      responseMimeType: 'application/json',
      responseSchema: {
        type: Type.ARRAY,
